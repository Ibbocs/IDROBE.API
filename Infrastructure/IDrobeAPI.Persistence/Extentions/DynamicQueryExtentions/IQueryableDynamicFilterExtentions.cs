...
namespace IDrobeAPI.Persistence.Extentions.DynamicQueryExtentions
{
    public static class IQueryableDynamicFilterExtentions
    {
        ...
        private static void GetFilters(Filter filter, IList<Filter> filters) //butun filterleri tek tek oxuyub liste elave elemek
        {
            filters.Add(filter);
            if (filter.Filters is not null && filter.Filters.Any()) //filtir icinde filtirler ola biler
